...
This function should only modify configuration layer settings."
  (setq-default
   ...
   dotspacemacs-additional-packages '(ddskk open-junk-file moom)
  ...
  (use-package moom
    ...
    (progn
      (spacemacs|define-transient-state moom
        :title "Moom"
        :doc "
[_+_/_=_] font increase [_-_] font decrease [_0_] reset font [_q_] quit"
        :global-key "<f12>"
        :bindings
        ("+" moom-font-increase)
        ("=" moom-font-increase)
        ("-" moom-font-decrease)
        ("0" moom-font-size-reset)
        ("q" nil :exit t))
      (spacemacs/set-leader-keys "<f12>" 'spacemacs/moom-transient-state/body)

      ;; Font size, either with ctrl + mouse wheel
      (global-set-key (kbd "<C-wheel-up>") 'moom-font-increase)
      (global-set-key (kbd "<C-wheel-down>") 'moom-font-decrease)))

  (use-package open-junk-file
    :bind (("C-x j" . open-junk-file))
    :config
    (setq open-junk-file-directory "~/scrapbox/"
          open-junk-file-format "~/scrapbox/%Y/%m/"))
  (use-package skk
    :ensure nil
    :config
